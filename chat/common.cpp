...
void database::createtable()
{
    ...
    if (tables.contains("users", Qt::CaseInsensitive)
            && tables.contains("friends", Qt::CaseInsensitive)
            )
    {
        //如果上述表名已存在,则不需要再次创建
    ...
    else
    {
        ...
        //创建客户信息表(编号, 自己, 好友, 主机, 好友IP)
        query.exec("create table friends(friendsid int primary key, name varchar, fname varchar, hname varchar, ipstr varchar)");
