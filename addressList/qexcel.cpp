...
void QExcel::setSheetName(int sheetIndex, QString sheetName)
{
    QAxObject * a = sheets->querySubObject("Item(int)", sheetIndex);
    a->setProperty("Name", sheetName);
}

void QExcel::copySheet(int srcIndex, int destIndex)
{
    QAxObject *src = sheets->querySubObject("Item(int)", srcIndex);
    src->dynamicCall("Copy(const QVariant&)", src->asVariant());

    src = sheets->querySubObject("Item(int)", srcIndex);
    QAxObject *dest = sheets->querySubObject("Item(int)", destIndex);
    dest->dynamicCall("Move(const QVariant&)", src->asVariant());
}
...
int QExcel::setXlsFilePath(QString xlsFilePath, QObject *parent)
{
    int successFlag = 0;
