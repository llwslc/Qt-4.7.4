...
void courseDialog::on_pushButton_clicked()
{
    ...
    //插入我的课程表
    idNum = 0;
    query.exec("select mycourseid from mycourse");
    while(query.next())
    {
        if(idNum < query.value(0).toInt())
            idNum = query.value(0).toInt();
    ...
    idNum++;
    query.exec("insert into mycourse values('" + QString::number(idNum, 10) + "', '"
               + ui->comboBox_2->currentText() + "', '"
               + ui->comboBox->currentText() + "', '"
               + QString::number(ui->comboBox_3->currentIndex()) + "', '"
               + QString::number(ui->comboBox_4->currentIndex()) + "')");
