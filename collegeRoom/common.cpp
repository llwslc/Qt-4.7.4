...
void database::createtable()
{
    ...
    if (tables.contains("person", Qt::CaseInsensitive)
            ...
            && tables.contains("libroom", Qt::CaseInsensitive)
            && tables.contains("libroomtime", Qt::CaseInsensitive)
            )
    {
        //如果上述表名已存在,则不需要再次创建
    ...
    else
    {
        ...
        //创建实验室表(编号,实验室)
        query.exec("create table libroom(libroomid int primary key, name varchar)");
        //创建实验室预约表(编号,实验室,开始时间,结束时间)
        query.exec("create table libroomtime(libroomtimeid int primary key, name varchar, timepre varchar, timenxt varchar)");
